...
A Windows tool that automatically trims excess transparent areas from PNG and WebP images via the context menu (right-click menu).
...
Windowsのコンテキストメニュー（右クリックメニュー）から、PNGおよびWebP画像の余分な透明部分を自動的にトリミングするツールです。
